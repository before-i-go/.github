...
# A script to find a string (case-insensitively) in various file types
# within a specific directory tree, combine the content of matching files,
# and split the result into 1MB chunks.
...
# 1. Validate Input (expects 2 arguments)
if [ "$#" -ne 2 ]; then
...
echo "🔎 Searching for: \"$SEARCH_STRING\" in '$SEARCH_DIR' (case-insensitive)..."
...
find "$SEARCH_DIR" -type f \( "${find_patterns[@]}" \) -print0 | while IFS= read -r -d '' file; do
    ...
    if grep -q -F -i -- "$SEARCH_STRING" "$TEMP_CONTENT_FILE"; then
...
# THIS IS THE FIX: The '-s' check ensures the combined file is not empty before splitting.
if [ "$found_count" -gt 0 ] && [ -s "$COMBINED_OUTPUT_FILE" ]; then
    echo "-----------------------------------------"
    ...
    echo "-----------------------------------------"
    echo "Search complete. The string was found, but no text content could be gathered."
fi
