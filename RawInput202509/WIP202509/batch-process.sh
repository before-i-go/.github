...
# batch-process.sh - Fully automated batch processing with automatic updates
# Processes file chunks and updates tracking document without manual intervention

set -e

# Usage function
usage() {
    echo "Usage: $0 <file_path> <tracking_document> [delay_seconds]"
    echo "  file_path: Full path to the file to process"
    echo "  tracking_document: Path to the Markdown file with checklist"
    echo "  delay_seconds: Seconds to wait between chunks (default: 3)"
    echo ""
    echo "Example: $0 ./research/rust-async.md A01SummaryProgress20250920.md 5"
    exit 1
}
...
FILE_PATH="$1"
TRACKING_DOC="$2"
DELAY=${3:-3}  # Default 3 seconds between chunks
...
echo "=== Batch Processing: $FILENAME ==="
...
update_checkbox() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "  ✓ Updated tracking document for chunk $chunk_num"
    else
        rm -f "$temp_file"
        echo "  ✗ Failed to update tracking document for chunk $chunk_num"
    fi
}
...
echo ""
echo "=== Batch Processing Complete for $FILENAME ==="
echo "All $CHUNKS chunks processed automatically."
echo "Tracking document updated for all chunks."